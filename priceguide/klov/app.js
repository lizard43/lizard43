...
/*
  Expects json in the same folder (ARRAY format):
...
const DATA_URL = "klov_lists.json";
...
function runSearch(rawQuery) {
  ...
  if (terms.length === 0) {
    ...
    filteredBlobs = gameBlobs.slice();
    filteredJoinedBlobs = gameJoinedBlobs.slice();
    matches = [];
    matchPos = 0;
  ...
  for (let i = 0; i < games.length; i++) {
    const g = games[i];
    const b = gameBlobs[i];
    const bJoined = gameJoinedBlobs[i];
...
async function loadData() {
  ...
  gameBlobs = games.map(buildBlob);
  gameJoinedBlobs = gameBlobs.map(normalizeNoSpace);
...
function getIncomingSearchParam() {
  let pending = "";
...
async function init() {
  ...
  try {
    ...
    const pending = getIncomingSearchParam();
